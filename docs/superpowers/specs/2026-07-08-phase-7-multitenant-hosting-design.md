...
- **API exposure:** the REST API is **private** (Railway internal network); only the
  authenticated web app reaches it. A separate key-gated public endpoint serves MCP/agents.
...
```
                 (public, GitHub-gated)          (private, internal network)
   Browser ─────▶  web  (Next.js + Auth.js) ────▶  api  (Hono, owns the DB) ──▶ volume:/data
                     │  session → X-User-Id +                    ▲                justdoit.db
                     │  X-Internal-Key (shared secret)           │                /data/files
                     └───────────────────────────────────────────┘
                                                                 │
   AI agent ─────▶  mcp  (public, X-API-Key per-user) ───────────┘  (or public api w/ key)
```

- **web** — the only public human surface. Auth.js (GitHub) gates every page via middleware.
  A same-origin proxy route forwards browser calls to the private API, injecting the user's
  identity server-side. The browser never holds a secret.
- **api** — the same Hono app + `core`, owning the SQLite file on the mounted volume. No public
  domain in hosted mode. Runs the reminder scheduler in-process (as today).
- **mcp** — public, per-user-key-gated HTTP transport for agent access (optional service; can
  also be reached via a key-gated path on the api).
...
2. Else valid `X-API-Key` → look up `api_keys.token_hash` → `user_id`. *(agents/MCP/scripts)*
3. Else, in local mode → `local-user`. In hosted mode → `401`.
...
- MCP and the public API resolve the key to a `userId`, so an agent only ever sees its owner's data.

## 7. Deployment (Railway)

One Railway project, three services from this monorepo:

| Service | Build | Public? | Notes |
| --- | --- | --- | --- |
| `web` | Dockerfile (Next.js standalone) | ✅ custom domain | Auth.js, proxy. |
| `api` | Dockerfile (tsx/node, `better-sqlite3`) | ❌ internal only | Mounts volume at `/data`; runs scheduler. |
| `mcp` | Dockerfile | ✅ (key-gated) | HTTP transport; optional. |
...
- **Private networking:** `web`/`mcp` reach `api` via its internal hostname; `api` has no public domain.
- **Env (hosted):** `JUSTDOIT_MODE=hosted`, `AUTH_SECRET`, `AUTH_GITHUB_ID`, `AUTH_GITHUB_SECRET`,
  `AUTH_ALLOWLIST`, `INTERNAL_API_URL`, `INTERNAL_API_SECRET`, plus the existing `JUSTDOIT_*`.
...
- **7b — GitHub auth & keys:** Auth.js + GitHub + allowlist in `web`; session→`userId`; the
  ...
  paths; per-user API-key create/revoke UI + hashing; MCP key→user resolution.
- **7c — Railway packaging:** Dockerfiles for `web`/`api`/`mcp`; `railway.json` + volume + private
  networking + env; Next.js standalone build; deploy runbook; smoke checklist.

## 9. Security considerations

- API never publicly routable in hosted mode; `X-Internal-Key` is defense-in-depth against a
  leaked internal URL.
